...
--------------------------------------------------------------------------------
-- 택배 상품상세 뷰
CREATE OR REPLACE VIEW DELIVERY_BOX1_VIEW
AS
...
select * 
from 테이블명 
where index in (select max(index) from 테이블명 group by 제목)
...
--○ 상품정보 뷰
CREATE OR REPLACE VIEW DELIVERY_BOX2_VIEW
AS
...
--○ 거래정보 뷰
CREATE OR REPLACE VIEW DELIVERY_BOX3_VIEW
AS
...
--○ 거래 완료 정보 뷰
CREATE OR REPLACE VIEW DELIVERY_BOX4_VIEW
AS
...
--○ 입찰내역 뷰
CREATE OR REPLACE VIEW DELIVERY_BOX5_VIEW
AS
...
-- 쿼리문
SELECT DISTINCT PD_NUM, DP.DIRE_PD_ID, U_ID, SUGGEST_PRICE, SUGGEST_TIME, HOPE_SATE
...
, rank() over (partition by PD_NUM order by suggest_price) AS RANK2
FROM TOTAL_DIRECT_VIEW
WHERE PD_NUM = 16)
...
SELECT PD_NUM, PD_ID, CATEGORY_NAME, PD_REGIT_DATE, U_ID, PD_TITLE, DEAL_TYPE, DEAL_STATUS
FROM TOTAL_PRODUCT_VIEW
WHERE PD_NUM IS NOT NULL
ORDER BY PD_NUM DESC
